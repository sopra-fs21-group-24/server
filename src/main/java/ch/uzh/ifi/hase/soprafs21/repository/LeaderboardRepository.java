...
public interface LeaderboardRepository extends JpaRepository<Leaderboard, String> {
    @Query("select l from Leaderboard l where l.gameMode = ?1 and l.score in(select max(n.score) from Leaderboard n where l.username = n.username) order by l.score desc")
